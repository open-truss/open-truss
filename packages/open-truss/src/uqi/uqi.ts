...
export type UqiMappedType = 'String' | 'Number' | 'Boolean' | 'BigInt' | 'Date' | 'Object'
...
interface UqiSettings<C, T> {
  ...
  query: (context: UqiContext<C, T>, query: string) => Promise<AsyncIterableIterator<UqiResult>>
...
function createClient<C, T>(og: UqiSettings<C, T>): UqiClient<C> {
  ...
  function buildRow(result: UqiResult): UqiScalar[] {
    return result.metadata.columns.map((column: UqiColumn, i: number) => {
      ...
      if (!type) { throw new Error(`Type ${column.type} is not mapped`) }
      const value = result.row[i]
      if (value === null) { return null }
      if (typeof value === 'string' && type === 'Number') { return Number(value) }
      if (typeof value === 'number' && type === 'String') { return String(value) }
      if (typeof value === 'bigint' && type === 'BigInt') { return BigInt(value) }
      if (typeof value === 'string' && type === 'Date') { return new Date(value) }
      return value
    })
  }

  function buildMetadata(metadata: { columns: UqiColumn[] }): { columns: UqiColumn[] } {
    return {
      ...metadata,
  ...
  return {
    ...
    async query(query: string) {
      if (!context.client) { throw new Error('Client is not set up') }
      const queryIterator = await og.query(context as unknown as UqiContext<C, T>, query)
